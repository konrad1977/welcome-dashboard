...
(defun welcome--calculate-padding-left ()
  ...
  (if-let* ((files welcome-recentfiles)
         (max-length (apply 'max (mapcar (lambda (path) (length (welcome--truncate-path-in-middle path welcome-path-max-length))) files)))
